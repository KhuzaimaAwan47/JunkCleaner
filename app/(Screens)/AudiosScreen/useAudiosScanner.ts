...
import { setAudiosResults } from '../../../redux-code/action';
import { scanAudios, type ScanProgress } from './AudiosScanner';
import type { CategoryFile } from '../../../utils/fileCategoryCalculator';

export const useAudiosScanner = () => {
  const dispatch = useDispatch();
  const [isScanning, setIsScanning] = useState(false);
  const [isRestoring, setIsRestoring] = useState(true);
  const [progress, setProgress] = useState<ScanProgress>({ total: 0, current: 0 });
  const [audios, setAudios] = useState<CategoryFile[]>([]);
  ...
  useEffect(() => {
    ...
    const loadSavedResults = async () => {
      try {
        ...
        console.error('Failed to load saved audios results:', error);
      } finally {
        if (isMounted) {
  ...
  const startScan = useCallback(async () => {
    ...
    try {
      const results = await scanAudios(
        (prog) => {
      ...
      if (!cancelRef.current) {
        setAudios(results);
        dispatch(setAudiosResults(results));
        ...
        if (results.length > 0) {
          try {
            await initDatabase();
            await saveAudiosResults(results);
            console.log(`Saved ${results.length} audios to database`);
          } catch (dbError) {
            console.error('Failed to save audios to database:', dbError);
            // Don't throw - allow scan to complete even if save fails
          }
        } else {
          // Even if no audios, clear old results
          try {
            await initDatabase();
            await saveAudiosResults([]);
          } catch (dbError) {
            console.error('Failed to clear audios in database:', dbError);
          }
        }
      } else {
      ...
      if (!cancelRef.current) {
        const message = err instanceof Error ? err.message : 'Failed to scan for audios';
        setError(message);
      }
    } finally {
      setIsScanning(false);
...
export default function UseAudiosScannerRoute(): null {
  return null;
}
